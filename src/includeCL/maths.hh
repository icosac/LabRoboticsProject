...
public:
  ...
  */
  ...
  char* getTypeName () const { ///<Returns a string (const char*) that tells the type of angle. The possibile types are RAD, DEG and INV for INVALID
...
public:
    ...
    */
    ...
    /*! \brief Constructors that takes a pointer array and the number of values
        and then stores them.

        \param[in] _n Number of obejctes to store.
        \param[in] arr Objects to store.
    */
    template <class T1>
    Tuple <T> (int _n, T1* arr){
      n=_n;
      for (int i=0; i<n; i++){
        elements.push_back((T)(arr[i]));
      }
    }
    ...
    /*! \brief Resize the tuple.
     *  \param[_size] the new size.
     *  \return The new size of the Tuple.
     */
    int size(const int _size) {
      if (_size!=this->size()){
        if (_size > this->size()) {
          this->add((T) 0);
        }
        else {
          for (int i = _size; i < this->size(); i++) {
            this->remove(i);
          }
        }
        n=_size;
    ...
    */
    ...
    //TODO This works only when both vector and tuple have the same type
    template <class T1>
    ...
    /*! \brief Overload of operator cast to pointer.
        \returns A pointer array of type T1.
    */
    template <class T1>
    operator T1* () const {
      T1* ret = new T1[this->size()*sizeof(T)];
    ...
    /*! \brief Copies a Tuple to this one. The original type is mantained.
     *
     * @tparam T1 The type of the Tuple to be copied.
     * @param in The Tuple to be copied.
     * @return The new Tuple.
     */
    template <class T1>
    Tuple<T> copy(const Tuple<T1> in){
      this->size(in.size());
      for(int i=0; i<in.size(); i++){
        this->set(i, (T)in.get(i));
      }
      return *this;
    }

    /*!\brief This function overloads the operator =. It simply calls the copy() function.
     *
     * @tparam T1 The type of the Tuple that has to be assigned.
     * @param in The Tuple that has to be assigned
     * @return The new Tuple assigned.
     */
    template <class T1>
    Tuple<T> operator= (const Tuple<T1> in){
      return copy(in);
    }
