...
namespace Planning {
    ...
    */
    vector<Configuration2<double> > planning(const Mat & img){
        ...
        #if defined WAIT || defined SHOW_MAP
            namedWindow("Empty map", WINDOW_NORMAL);
            imshow("Empty map", imageMap);
            mywaitkey();
        #endif
        ...
        for(int i=0; i<n-1; i++){
            ...
            for(int j=i+1; j<n; j++){
                ...
                for(int q=0; q<(int)(vvvp[i][j].size())-1; q++){
                    d = vvvp[i][j][q].distance( vvvp[i][j][q+1] );
                    dist += (vvvp[i][j].size()==2 ? 99999.0 : d);
                    if(i==0 && j==(n-1) && dist>99998.0){
                        throw MyException<string>(EXCEPTION_TYPE::GENERAL, "Even the best path can't reach the gate. I'm so sorry...\n\t", __LINE__, __FILE__);
                    }
                }
                vvDist[i][j] = dist;
                vvDist[j][i] = dist;
                cout << (int)vvDist[i][j] << "\t" << flush;
            }
        }
        cout << endl;
    ...
    */
                            ...
                            double ** distances, Point2<int> ** parents)
    {
        ...
        if(found==0){
            #ifdef BEST
                computedParents.push_back(startP);
    ...
    bool compute_roundabout_dubins( DubinsSet<double>& new_DS,
        ...
        for (vC_id=scrap; (vC_id<(vC.size()-scrap) && !ok); vC_id++){
            ...
            for (int i=0; i<(offset.toRad()*2.0/(10.0*::DEGTORAD)+1) && !ok; i++){
                start.offset_angle(Angle(10.0*::DEGTORAD, Angle::RAD));
